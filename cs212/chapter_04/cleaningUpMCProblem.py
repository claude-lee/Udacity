...
def mc_problem2(start=(3, 3, 1, 0, 0, 0), goal=None):
    ...
    return shortest_path_search(start, csuccessors, is_goal_mc) # <== insert arguments here

def is_goal_mc(start, state):
    m1, c1, b1, m2, c2, b2 = start
    return state == (m2, c2, b2, m1, c1, b1)

def shortest_path_search(start, successors, is_goal):
    """Find the shortest path from start state to a state
    such that is_goal(state) is true."""
    if is_goal(start, start):
        return [start]
    explored = set()
    frontier = [ [start] ] 
    while frontier:
