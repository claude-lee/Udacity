...
def bsuccessors(state):
    ...
    here, there, t = state
    if 'light' in here:
	    return {(here.difference([a,b,'light']), here.union([a,b,'light']), t+max(a,b)) : 
			(a,b,'->') for a in here if a is not 'light' for b in here if b is not 'light'}
    elif 'light' in there:
        return {(here.union([a,b, 'light']), here.difference([a,b, 'light']), t +max(a,b)) : 
    		(a, b, '<-') for a in there if a is not 'light' for b in there if b is not 'light'}
