...
def inverse(f, delta = 1/1024.):
    ...
    inverse, picking the closest value to the inverse, within delta."""
    def f_1(y):
        low, high = find_boundaries(f,y)
        return binary_search(f, y, low, high, delta)
    return f_1

def find_boundaries(f, y):
    "Find values low, high such that f(low) <= y <= f(hi)/"
    x = 1
    while f(x) < y:
        x = 2*x
...
def binary_search(f, y, low, high, delta):
    ...
    while low <= high:
        ...
        if f(middle) < y:
            ...
            return middle
    return low if (y-f(low) < f(high)-y) else high
